"""Synthetic dataset generator — LOCAL DEVELOPMENT / TESTING ONLY.

The real Kaggle ULB ``creditcard.csv`` (mlg-ulb/creditcardfraud) requires a
Kaggle account to download and is ~150 MB, so it is not committed. This module
fabricates a dataset with the *exact same schema* (Time, V1..V28, Amount, Class)
and a realistic ~0.17% fraud rate, so offline local runs and isolated tests can
exercise the pipeline without the real file. CI always fetches the real dataset.
...
_SYNTHETIC_SOURCE = "SYNTHETIC — generated by src/simulate_data.py (local dev/test only)"
